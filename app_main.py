...
app = FastAPI(
    title="CEMIG OCR API - Survey123 RC13.3",
    version="1.0.0-RC13.3",
    description="CEMIG OCR API RC13.3: OCR CEMIG SIM / GD + detecção GD na conta principal.",
)
...
def raiz():
    return {
        "status": "ok",
        "versao": "1.0.0-RC13.3",
        "mensagem": "FastAPI iniciou sem carregar Paddle/PaddleOCR. RC13.3 com OCR CEMIG SIM / GD + detecção GD na conta principal.",
    }
...
def _selecionar_anexos_gd_survey123(anexos):
    ...
    localizada e registrada em log nesta RC13.3, para confirmar o formato do payload.
    """
...
def _baixar_anexo_survey123(anexo: dict, token: str | None) -> Path:
    ...
    req = urllib.request.Request(url_download, headers={"User-Agent": "cemig-ocr-api/1.0.0-RC13.3"})
    try:
...
def _arcgis_request_json(url: str, params: dict | None = None, timeout: int = 60):
    ...
    req = urllib.request.Request(url_final, headers={"User-Agent": "cemig-ocr-api/1.0.0-RC13.3"})
    try:
...
def _arcgis_post_form_json(url: str, data: dict, timeout: int = 90):
    ...
    req = urllib.request.Request(
        ...
        data=encoded,
        headers={
            "User-Agent": "cemig-ocr-api/1.0.0-RC13.3",
...
def _montar_contexto_gd(resultado: dict, resultado_sim: dict | None, eh_gd: bool | None, anexo_sim_presente: bool):
    ...
    # RC13.3: a conta principal pode indicar GD mesmo quando o usuário marcou
    # "Não" no formulário. Se a fatura SIM foi processada, a API também confirma
    # o contexto GD; caso contrário, usa a detecção da conta principal.
    gd_detectado_principal = resultado.get("gdDetectado")
    if gd_detectado_principal is True or resultado_sim:
        gd_detectado_api = True
    elif gd_detectado_principal is False:
        gd_detectado_api = False
    else:
        gd_detectado_api = None
...
def _processar_survey123_em_background(payload: dict, meta: dict | None = None):
    ...
    RC13.3: processa a conta CEMIG principal e, quando existir, a segunda fatura
    ...
    try:
        ...
        print(
            f"[SURVEY123 RC13.3] início objectId={object_id} globalId={global_id} "
            f"layerId={layer_id} eh_gd_raw={eh_gd_raw!r} eh_gd={eh_gd} "
            f"anexos={len(anexos)} bytes={meta.get('body_bytes')}",
            flush=True,
        )
        print(
            f"[SURVEY123 RC13.3] anexos_payload objectId={object_id} "
            f"resumo={json.dumps(resumo_anexos, ensure_ascii=False)}",
            flush=True,
        )

        if not anexos:
            print(f"[SURVEY123 RC13.3] erro objectId={object_id}: nenhum anexo encontrado", flush=True)
            return
        ...
        print(
            f"[SURVEY123 RC13.3] selecao_anexos objectId={object_id} "
        ...
        if not anexo:
            print(f"[SURVEY123 RC13.3] erro objectId={object_id}: anexo principal não localizado", flush=True)
            return

        print(
            f"[SURVEY123 RC13.3] baixando anexo principal objectId={object_id} "
        ...
        print(f"[SURVEY123 RC13.3] anexo principal baixado objectId={object_id} bytes={caminho.stat().st_size}", flush=True)
        ...
        if anexo_sim:
            print(
                f"[SURVEY123 RC13.3] baixando fatura SIM objectId={object_id} "
            ...
            print(f"[SURVEY123 RC13.3] fatura SIM baixada objectId={object_id} bytes={caminho_sim.stat().st_size}", flush=True)
            ...
            print(
                f"[SURVEY123 RC13.3] fatura SIM processada objectId={object_id} "
            ...
            print(
                f"[SURVEY123 RC13.3] alerta_gd objectId={object_id}: "
        ...
        print(
            f"[SURVEY123 RC13.3] processado objectId={object_id} "
            ...
            f"gd_detectado_api={gd_contexto.get('gd_detectado_api')} "
        ...
        print(
            f"[SURVEY123 RC13.3] erro objectId={info.get('objectId') if isinstance(info, dict) else None}: "
            f"{type(exc).__name__}: {exc}",
            flush=True,
        )
...
async def survey123_webhook(request: Request, background_tasks: BackgroundTasks):
    ...
    print(
        f"[SURVEY123 RC13.3] recebido objectId={info.get('objectId')} "
    ...
    print(
        f"[SURVEY123 RC13.3] anexos_recebidos objectId={info.get('objectId')} "
        f"resumo={json.dumps(resumo_anexos, ensure_ascii=False)}",
        flush=True,
    )
    _logar_diagnostico_detalhado_anexos("[SURVEY123 RC13.3 POST]", info.get('objectId'), payload, info)
